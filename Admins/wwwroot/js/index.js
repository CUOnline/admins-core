    ...
    let app = new Vue({
        ...
        mounted: function () {
            return __awaiter(this, void 0, void 0, function* () {
                if (authorized) {
                    // Get the schools
                    let response = yield axios.get(webApiUrl + '/api/v1/accounts/1/sub_accounts?per_page=100&access_token=' + tokenStr);
                    ...
                    for (let i = 0; i < this.subAccounts.length; ++i) {
                        let adminsResponse = yield axios.get(webApiUrl + '/api/v1/accounts/' + this.subAccounts[i].id + '/admins?access_token=' + tokenStr);
                        ...
                        for (let j = 0; j < this.subAccounts[i].admins.length; ++j) {
                            ...
                            if (!admin.email || admin.email === '') {
                                ...
                                if (existingAdminData) {
                                    ...
                                    let profileResponse = yield axios.get(webApiUrl + '/api/v1/users/' + admin.user.id + '/profile?access_token=' + tokenStr);
                                    ...
                                    let activityResponse = yield axios.get(webApiUrl + '/api/v1/users/' + admin.user.id + '/page_views?per_page=1&access_token=' + tokenStr);
